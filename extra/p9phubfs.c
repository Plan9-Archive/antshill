...
int trunca;						/* In trunc mode only new data is sent, not buffered */
...
wrsend(Hub *h)
{
	...
	for(i = h->qwans; i <= h->qwnum; i++){
		...
		h->buckfull += count;
		r->fid->file->dir.length = h->buckfull;
		r->ofcall.count = count;
		h->wstatus[i] = DONE;
		if((i == h->qwans) && (i < h->qwnum))
			h->qwans++;
...
Buffersize == %ulld \n", srvname, paranoia, freeze, trunca, applylimits, bucksize);
